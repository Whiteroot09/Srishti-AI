...
def log_in(window):
    ...
    def countdown():
        def sorry_log(root):
            online("Sorry for Eruption. I think this is the fault of the Internet on the camera. please try to log in again.")
            close_log(root)
        ...
        online("After 5 second, The password section will appear, and you need to provide the password, for your user account")
        while i < 5:
            time.sleep(0.5)
            i += 1
        ...
        if pack == None or pack == False:
            ...
            data.config(text="You need to go back and log in again!!\nOr if you not sign up in SIGN UP",
                        font=("Arial", 10, "bold"), fg="#B03A2E")
            online("Face ID not found")
            online("You need to go back and log in again, Or if you not sign up in SIGN UP")
        else:
            userid = pack[0]
            name = pack[1]
            gender = pack[2]
            ...
            accpet()
            time.sleep(2)
            data.config(text=f"Is that You, {add}?", font=("Arial", 10, "bold"), fg="#27AE60")
            ...
            cancel = tk.Button(confirm_layer, width=10, text="No", bg="#A9DFBF", bd=0,
